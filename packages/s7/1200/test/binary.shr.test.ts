...
const TestProgram = () => {
    const fb = new Fb({
        ...
        body() {
            return [
                new Assign(this.temp.test_byte, new Shr(new Byte(8), new Byte(2))),
                new UnitTest("Byte shr", this.temp.test_byte, "=", 2),

                new Assign(this.temp.test_word, new Shr(new Word(8), new Word(2))),
                new UnitTest("Word shr", this.temp.test_word, "=", 2),
            ]
        }
    })
        ...
        describe("Simulation tests", async () => {
            ...
            describe("Compiler output", () => {
                ...
                it("output", () => {
                    ...
                    expect(output["file:///Shr_Fb"]).to.toMatchObject([
                        "FUNCTION_BLOCK \"Shr_Fb\"",
                        ...
                        "	END_VAR",
                        "BEGIN",
                        "	#test_byte := SHR(IN := BYTE#16#8, N := 16#2);",
                        "	#test_word := SHR(IN := WORD#16#8, N := 16#2);",
                        "",
                        "END_FUNCTION_BLOCK"
                    ])
                })

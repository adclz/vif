<template>
  ...
  >
    <div class="flex flex-col gap-4 w-full items-center h-max">
      ...
      <div v-if="protocol" class="flex flex-col gap-4 w-full items-center h-max">
      <div class="flex flex-row gap-2 items-end self-baseline">
        <va-input
            label="Agent Url (optional)"
            v-model="CompilerURL"/>
        <va-button @click="openAgent" class="w-max h-max">
          Open Agent
        </va-button>
        <span>{{protocol}}</span>
      </div>
      ...
      <span v-else>The provider does not define an external agent.</span>
      <va-avatar
...
const openAgent = () => window.open(`${protocol.value}:--web`, "_self")

...
- It works on any IDE as far as this one supports Javascript / Typescript (which means pretty much all IDEs).
...
Since we working with javascript, you might be wondering how vif handles low level types and makes sure operations are valid.
...
In fact our previous example could be refactored in many, many ways because Javascript is the biggest language ecosystem
ever created.
